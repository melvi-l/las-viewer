...
#define NEVER(cond) ((cond) ? (assert(0), 1) : 0)
#define ALWAYS(cond) ((cond) ? 1 : (assert(0), 0))
...
#define ARENA_BLOCK_DATA(b) ((u8 *)((b) + 1))
...
static inline bool arena_try_stitch(Arena *arena, void *ptr, u64 size, u64 grow) {
    ArenaBlock *block = arena->current;

    u8 *ptr_end = (u8 *)ptr + size;

    u8 *block_end = ARENA_BLOCK_DATA(block) + block->used;
...
static inline void str_builder_grow(Arena *arena, StrBuilder *builder, u64 new_capacity) {
...
static inline Str str_builder_push_str(Arena *arena, StrBuilder *builder, Str *str) {
...
// @deserialized
static inline u64 str_deserialized(Str str, u64 off, void *read_dst, u64 read_size, u64 granularity) {
...
#define str_deserialized_array(string, off, ptr, count) \
    str_deserialized((string), (off), (ptr), sizeof(*(ptr)) * (count), sizeof(*(ptr)))

#define str_deserialized_struct(string, off, ptr) \
    str_deserialized_array(string, off, ptr, 1)

static inline u64 str_deserialized_str(Arena *arena, Str string, u64 off, Str *str) {
    u64 _off = off;
...
// @ring
static inline u64 read_ring(u8 *ring, u64 ring_position, u64 ring_size, u8 *data, u64 size) {
...
static inline u64 write_ring(u8 *ring, u64 ring_position, u64 ring_size, u8 *data, u64 size) {
...
void *arena_alloc(Arena *arena, size_t size) {
    ...
    if (new_end <= block->size) {
        void *ptr = ARENA_BLOCK_DATA(block) + align_used;
    ...
    void *ptr = ARENA_BLOCK_DATA(new_block);
    new_block->used += size;
